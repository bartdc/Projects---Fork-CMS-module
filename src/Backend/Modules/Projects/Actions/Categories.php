...
/**
 * This is the categories action, it will display the overview of project categories.
 *
...
class Categories extends BackendBaseActionIndex
{
	...
	protected function parse()
	{
		...
		else $this->tpl->assign('showProjectsAddCategory', false);
	}
	...
	public static function setClickableCount($count, $link)
	{
		...
		if($count > 1) return '<a href="' . $link . '">' . $count . ' ' . BL::getLabel('Projects') . '</a>';
		if($count == 1) return '<a href="' . $link . '">' . $count . ' ' . BL::getLabel('Project') . '</a>';
		else return BL::getLabel('NoProjects');
	}
}
